...
@section('title')
<title>Contrat - Ajouter Contrat</title>
@endsection
...
<body class="hold-transition sidebar-collapse layout-top-nav layout-footer-fixed">
    ...
    <div class="content">
      <div class="container-fluid" style="height: 70vh;">
            ...
            <div class="card">
              <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" type="text" value="{{$contrat->name}}" readonly>
                </div>
                <div class="form-group">

                    <button type="button" class="btn btn-default mr-1" data-toggle="modal" data-target="#modal-ajouter-partie-contrat">Ajouter une Partie</button>
                ...
                <div class="form-group">
                    ...
                    <div class="card card-success collapsed-card">
                        <div class="card-header">
                            <h2 class="card-title"> {{ $partieContrat->titre }} </h2>
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="card_title_value_1" value="{{ $partieContrat->titre }}" readonly>
                            </div>

                            {{-- Si partie d'un type contrat n'a pas d'articles --}}
                            {{-- @if(!$partieContrat->has_articles)
                            <div class="form-group">
                                <label for="">Contenu: </label>
                                <textarea id="summernote_partie{{ $loop->index }}" name="contenu_partie_1" placeholder="" value="1">
                                    {{ $partieContrat->contenu }}
                                </textarea>
                            </div>
                            @endif --}}

                            {{-- Modal editer contenu type contrat partie --}}
                            {{-- @if (!empty($partieContrat->contenu))@endif --}}
                            <div class="form-group">
                                <label for="">
                                    Contenu:
                                    <div class="btn-group">
                            ...
                            {{-- @if ($partieContrat->has_articles) --}}
                            {{-- <div class="form-group">
                                <button type="button" class="btn btn-default mr-1 float-right" data-toggle="modal" data-target="#modal-default-article{{ $partieContrat->id }}">Ajouter un Article</button>
                            </div> --}}
                            {{-- @endif --}}
                            ...
                            @if ($partieContrat->id == $articleContrat->contrat_partie_id)
                                <div class="form-group">
                                    <label for="">
                                        {{ $articleContrat->intitule_article }} : {{ $articleContrat->titre_article }}
                                    ...
                                    {{-- <textarea id="partie_contrat_article{{ $loop->index }}" name="partie_contrat_article{{ $loop->index }}" placeholder="" value="1">
                                        {{ $articleContrat->contenu }}
                                    </textarea> --}}

                                </div>
                            @endif
                                {{-- {{$articleContrat->intitule_article}} --}}
                            @endforeach

                            {{-- @foreach (articleTypeContrats as articleTypeContrat)
                                @if ($partieContrat->id == $articleContrat->type_contrat_partie_id)
                                <div class="form-group">
                                    <label for="">{{ $articleContrat->intitule_article }}</label>
                            ...
                            <div class="form-group">
                                <a href="{{ route('type_contrats.add_article', ['type_contrat_id' => $contrat->id, 'type_contrat_partie_id' => $partieContrat->id ]) }}"><button type="button" class="btn btn-default mr-1 float-right">Ajouter un Article</button></a>
                            </div>
        ...
        <div class="modal fade" id="modal-ajouter-partie-contrat">
          <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="put" action=" {{ route('type_contrats.update', ['type_contrat' => $contrat->id]) }} ">
                        @csrf
                        <div class="form-group">
                            <label for="">Titre :</label>
        ...
        @foreach($partieContrats as $partieContrat)
        {{-- @if ($partieContrat->has_articles) --}}
            <div class="modal fade" id="modal-default-article{{ $partieContrat->id }}">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                    ...
                    <div class="modal-body">
                        ...
                        <form method="put" action=" {{ route('type_contrats.update', ['type_contrat' => $contrat->id]) }} ">
                            ...
                            <input type="hidden" name="type_partie_id" value="{{ $partieContrat->id }}">
        ...
        {{-- Edit Type Contrat Content --}}
            <div class="modal fade" id="modal-default-edit-type-contrat-partie-content{{$loop->index}}">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="put" action="{{route('type_contrats.update', $partieContrat->id)}}">
                                @csrf
                                {{-- A revoir --}}
                                <input type="hidden" name="edit_type_contrat_partie_id" value="{{$partieContrat->id}}">
                                <div class="form-group">
                                    <label for="">Titre :</label>
                                    <input type="text" class="form-control" name="edit_partie_titre" placeholder="Ex. Définition des conditions d'existence" value="{{$partieContrat->titre}}" readonly>
                                </div>
                                <div class="form-group" id="hideValuesOnSelect">
                                    <label>Contenu :</label>
            ...
            @foreach($articleContrats as $articleContrat)
            @if ($articleContrat->type_contrat_partie_id === $partieContrat->id)
                {{-- Edit Type Contrat Content --}}
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form method="put" action="{{route('type_contrats.update', $partieContrat->id)}}">
                                    @csrf
                                    {{-- A revoir --}}
                                    <input type="hidden" name="t_contrat_id" value="{{$contrat->id}}">
                                    <input type="hidden" name="edit_type_contrat_article_partie_id" value="{{$partieContrat->id}}">
                                    <input type="hidden" name="edit_type_contrat_article_id" value="{{$articleContrat->id}}">
                                    <input type="hidden" name="edit_type_contrat_article_name" value="{{$articleContrat->titre_article}}">

                                    <div class="form-group">
                                        <label for="">Titre de l'article:</label>
                                        <input type="text" class="form-control" name="edit_article_titre" placeholder="Ex. Objet" value="{{$articleContrat->titre_article}}">
                                    </div>
                                    <div class="form-group" id="hideValuesOnSelect">
                                        <label>Contenu :</label>
